...
use atlas_core::net::rpc::wire::{AtlasWireHeader, AtlasWireKind, AtlasWireRequest};
...
fn main() {
    ...
    let request = AtlasWireRequest {
        ...
        payload: RegisterReq { account: "val".into(), password: "val".into() },
    };
    ...
    let payload_bytes =
        rmp_serde::to_vec(&request.payload).expect("payload encode failed");
    ...
    // 修改 kind
    wire_mut[16] = AtlasWireKind::Response as u8;
    ...
    let decoded_payload: RegisterReq =
        rmp_serde::from_slice(&buf).expect("payload decode failed");
