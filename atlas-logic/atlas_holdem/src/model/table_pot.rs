...
pub fn build_pots(players: &[Player]) -> Vec<SidePot> {
    ...
    let mut bet_levels: Vec<u64> = players
        .iter()
...
pub fn settle_pots(players: &mut [Player], pots: Vec<SidePot>) {
    for pot in pots {
        ...
        let max_rank = pot.contenders
            .iter()
            .map(|&i| players[i].cards_rank.unwrap())
            .max()
            .unwrap();
        // 找所有并列赢家
        let winners: Vec<usize> = pot
            .contenders
            .into_iter()
            .filter(|&i| players[i].cards_rank.unwrap() == max_rank)
            .collect();
        ...
        for idx in winners {
            players[idx].balance += share;
            if idx == 0 {                                       // ②
                players[idx].balance += odd;                    // ③ odd chip 给第一个赢家
            }
            players[idx].win = true;
        }
    }
}
